...
public class Chunk
{
    ...
    public Chunk(int index, Location location, int timestamp, byte[] payload) throws IOException
    {
        ...
        {
            NBTReader reader = new NBTReader(NBTReader.byteArrayToDataInputStream(this.payload.getDecompressedData()));
            ...
            } else if (root.getTagByName("pos") instanceof Tag_Int_Array position)
            {
                // If pos is found, we are dealing with a poi file
                this.x = position.getData()[0];
                this.z = position.getData()[2];
            } else
            {
                // Else it must be a region file
    ...
    public boolean hasOwnableEntities() throws IOException
    {
        ...
        Tag_Compound root = new NBTReader(NBTReader.byteArrayToDataInputStream(this.getPayload().getData())).read();
        if (root.getTagByName("Owner") != null) ownable = true;
        if (root.getTagByName("Target") != null) ownable = true;
        return ownable;
    }
