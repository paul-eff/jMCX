...
public class AnvilReader implements IAnvilReader
{
    ...
    public Optional<IChunk> readChunk(int chunkX, int chunkZ) throws IOException
    {
        ...
        {
            throw new IOException("Failed to read chunk at coordinates (" + chunkX + ", " + chunkZ + "): " + e.getMessage(), e);
        }
    }
    ...
    private Location parseAndValidateLocation(byte[] locationBytes, int chunkIndex, long fileSize) throws IOException
    {
        ...
        {
            // Validate minimum sector offset
            ...
            {
                throw new IOException(
                    String.format("Invalid chunk %d: sector count %d must be between 1 and 255", 
                        chunkIndex, sectorCount)
                );
            }
            
            // Validate chunk placement using AnvilUtils
    ...
    private int[] readTimestampTable() throws IOException
    {
        ...
        for (int i = 0; i < CHUNKS_PER_REGION; i++)
        {
            ...
            if (timestamps[i] > maxFutureEpoch)
            {
                ...
                System.err.println(
                    String.format("Warning: chunk %d has timestamp %d which is more than 100 years in the future", 
                        i, timestamps[i])
                );
            }
        }
